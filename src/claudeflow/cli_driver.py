...
class CliDriver:
    ...
    def _store_session(self, session_id: str, process: subprocess.Popen, prompt: str = ""):
        """
        存储会话信息
    ...
    def start_session(self, prompt: str) -> Tuple[subprocess.Popen, str]:
        """
        启动新的CLI会话
        ...
        """
        cmd = self._build_start_command(prompt)
        ...
        session_id = self._extract_session_id(first_line)

        if session_id:
            self._store_session(session_id, process, prompt)
